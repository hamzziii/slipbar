...
숨기면 macOS 27의 기본 버튼 **«**(가려진 메뉴 막대 항목 보기)가 나타납니다. 누르면 숨긴 아이콘을 잠깐 꺼내 볼 수 있습니다.
...
Slip은 ●› 바로 왼쪽에 자기 `NSStatusItem`(spacer)을 하나 두고, 숨길 때 그 `length`를 키워 왼쪽 아이콘들을 **밀어냅니다.** macOS 27에서는 밀려난 아이콘이 시스템의 « 오버플로 안으로 들어갑니다. 비공개 API·SIP 해제·화면 녹화 권한 모두 쓰지 않습니다.
...
| 현상 | Slip의 대응 |
|------|-------------|
| spacer가 너무 길면(화면 가운데를 한참 넘으면) 앞에 있는 앱의 메뉴가 길 때 무시되고 아이콘이 **다시 튀어나옴** (고정 45%·"10000pt로 밀기" 모두 해당) | ●›의 실제 위치에서 **화면 폭 53% 지점까지만** 밀도록 매번 계산 → 아이콘은 macOS « 오버플로로 들어감 |
